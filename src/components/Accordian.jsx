  ...
  return (
    <div className='p-6'>
        ...
        <button onClick={() => setAccordionOpen(!accordionOpen)} className="flex justify-between w-full">
            <span className='font-semibold hover:font-black hover:text-my-grayish-purple'>
                {title}
            </span>
            {accordionOpen ? <span>
        ...
        <div className={`grid overflow-hidden transition-all duration-300 ease-in-out text-slate-600 texxt-sm ${accordionOpen ? 'grid-rows-[1fr] opacity-100 text-zinc-300' : 'grid-rows-[0fr] opacity-0'}`}>
